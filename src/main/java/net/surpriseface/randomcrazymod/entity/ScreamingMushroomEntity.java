...
package net.surpriseface.randomcrazymod.entity;
...
import net.surpriseface.randomcrazymod.procedures.ScreamingMushroomOnInitialEntitySpawnProcedure;
import net.surpriseface.randomcrazymod.procedures.ScreamingMushroomOnEntityTickUpdateProcedure;
...
public class ScreamingMushroomEntity extends Monster implements IAnimatable {
	...
	protected void registerGoals() {
		...
		this.goalSelector.addGoal(3, new MeleeAttackGoal(this, 3.2, true) {
			@Override
	...
	public SoundEvent getHurtSound(DamageSource ds) {
		return ForgeRegistries.SOUND_EVENTS.getValue(new ResourceLocation("block.fungus.place"));
	}
	...
	public SpawnGroupData finalizeSpawn(ServerLevelAccessor world, DifficultyInstance difficulty, MobSpawnType reason, @Nullable SpawnGroupData livingdata, @Nullable CompoundTag tag) {
		...
		ScreamingMushroomOnInitialEntitySpawnProcedure.execute(this);
		return retval;
	}
	...
	public void baseTick() {
		super.baseTick();
		ScreamingMushroomOnEntityTickUpdateProcedure.execute(this.level, this.getX(), this.getY(), this.getZ(), this);
	...
	private <E extends IAnimatable> PlayState movementPredicate(AnimationEvent<E> event) {
		if (this.animationprocedure.equals("empty")) {
			if ((event.isMoving() || !(event.getLimbSwingAmount() > -0.15F && event.getLimbSwingAmount() < 0.15F))
					...
					&& !this.isAggressive()) {
				event.getController().setAnimation(new AnimationBuilder().addAnimation("walk/jump", EDefaultLoopTypes.LOOP));
				return PlayState.CONTINUE;
			}
			if (this.isDeadOrDying()) {
				event.getController().setAnimation(new AnimationBuilder().addAnimation("dig", EDefaultLoopTypes.PLAY_ONCE));
				return PlayState.CONTINUE;
			}
			if (this.isSprinting()) {
				event.getController().setAnimation(new AnimationBuilder().addAnimation("walk/jump", EDefaultLoopTypes.LOOP));
			...
			if (this.isAggressive() && event.isMoving()) {
				event.getController().setAnimation(new AnimationBuilder().addAnimation("scream", EDefaultLoopTypes.LOOP));
				return PlayState.CONTINUE;
			}
